import {
  ...
  SimpleChange, ElementRef, Inject,
  ViewEncapsulation
} from '@angular/core';
import {
  IMyDate, IMyMonth, IMyWeek,
  IMyDayLabels, IMyMonthLabels,
  IMyLocales, IMyOptions
...
export class MyDatePicker implements OnInit, OnChanges {
  ...
  defaultDate: IMyDate = { year: 0, month: 0, day: 0, hours: 0, minutes: 0, meridian: 'AM' };
  selectedDate: IMyDate = { year: 0, month: 0, day: 0, hours: 0, minutes: 0, meridian: 'AM' };
  ...
  ngOnInit() {
    ...
    let optionprops = ['dayLabels', 'monthLabels',
      'dateFormat', 'todayBtnTxt', 'firstDayOfWeek',
      'sunHighlight', 'disableUntil', 'disableSince',
      'disableWeekends', 'height', 'width', 'inline'];
  ...
  openBtnClicked(): void {
    ...
    if (this.showSelector) {
      ...
      if (this.selectedDate.year === 0 &&
        ...
        if (this.defaultDate.year === 0 &&
          this.defaultDate.month === 0) {
  ...
  selectDate(date: any): void {
    ...
    let epoc = new Date(
      ...
      this.time.getMilliseconds()).getTime() / 1000.0;
    this.dateChanged.emit(
      {
  ...
  changed(evt) {
    ...
    this.selectionDayTxt = this.formatDate(this.selectedDate);
    let epoc = new Date(this.selectedDate.year,
      this.selectedDate.month,
      this.selectedDate.day,
      this.time.getHours(),
      this.time.getMinutes(),
  ...
  private _parseDate(ds: string): IMyDate {
    let rv: IMyDate = { day: 0, month: 0, year: 0, hours: 0, minutes: 0, meridian: 'AM' };
    if (ds !== '' && ds !== ' ') {
      let fmt = this.options && this.options.dateFormat !== undefined
